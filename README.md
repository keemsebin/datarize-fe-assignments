...
- [ ] 한 달 동안 발생한 구매 데이터를 기준으로 가격대별 구매 빈도를 집계한다.
- [ ] 가격대는 **2만원 이하부터 10만원 이상까지, 1만원 단위**로 구분한다.
- [ ] 각 가격대별 구매 횟수를 계산한다.
- [ ] 구매 빈도 데이터를 **바 차트 형태로 시각화**한다.
- [ ] 날짜 선택 UI를 제공한다.
  - [ ] 시작 날짜와 종료 날짜를 선택할 수 있다.
  - [ ] 선택한 기간에 따라 차트 데이터가 갱신된다.

---

### 가장 많이 구매한 고객 목록 및 검색 기능

- [ ] 최근 한 달 동안의 구매 데이터를 기준으로 고객별 구매 정보를 집계한다.
- [ ] 고객 목록에 다음 정보를 표시한다.
  - [ ] 고객 ID
  - [ ] 고객 이름
  - [ ] 총 구매 횟수
  - [ ] 총 구매 금액
- [ ] 기본 정렬 기준은 **고객 ID 오름차순**이다.
- [ ] 정렬 옵션을 제공한다.
  - [ ] 구매 횟수 기준 오름차순
  - [ ] 구매 횟수 기준 내림차순
- [ ] 고객 이름을 기준으로 검색할 수 있다.
  - [ ] 검색어 입력 시 이름이 포함된 고객만 목록에 노출된다.

---

### 고객 ID 기반 상세 구매 내역 조회

- [ ] 고객 목록에서 특정 고객 Row를 클릭할 수 있다.
- [ ] 클릭 시 해당 고객의 상세 구매 내역을 조회한다.
- [ ] 상세 구매 내역에는 다음 정보가 포함된다.
  - [ ] 구매 날짜
  - [ ] 구매한 제품 목록
  - [ ] 각 제품의 가격
  - [ ] 상품 썸네일 이미지
- [ ] 조회된 상세 구매 내역은 고객 ID 기준으로 필터링된다.
