...
서버 상태 관리는 TanStack Query를 사용해 로딩, 에러, 성공 상태를 선언적으로 관리했습니다. `throwOnError: true` 옵션을 설정하여 에러 발생 시 ErrorBoundary에서 일괄 처리할 수 있도록 구성했습니다.
...
errorMessage를 unknown 타입으로 받아 타입 가드로 안전하게 처리하며, 변환된 메시지는 ApiError 클래스에 담겨 ErrorBoundary로 전달됩니다. 사용자는 명확한 한국어 메시지를 통해 문제를 이해하고 적절히 대응할 수 있습니다.
...
> Storybook 설정 과정에서 npm version을 22로 설정했습니다.
